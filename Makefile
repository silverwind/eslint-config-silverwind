...
lint: node_modules build
	node bin/eslint-silverwind.js -c dist/index.js --color .
	pnpm exec tsgo

.PHONY: lint-fix
lint-fix: node_modules build
	node bin/eslint-silverwind.js -c dist/index.js --color --fix .
	pnpm exec tsgo

.PHONY: test
test: node_modules build
	node bin/eslint-silverwind.js -c dist/index.js --color tests
	pnpm exec vitest
